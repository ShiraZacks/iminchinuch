...
const HOLIDAY_IGNORE_MASK = core.flags.DAF_YOMI | core.flags.OMER_COUNT | core.flags.SHABBAT_MEVARCHIM | core.flags.MOLAD | core.flags.USER_EVENT | core.flags.HEBREW_DATE | core.flags.MISHNA_YOMI | core.flags.MODERN_HOLIDAY | core.flags.YERUSHALMI_YOMI;
...
function getLeyningForParshaShabbatOnly(parsha) {
  ...
  const result = {
    name: {
      en: name,
      he: parshaNameArray.map(s => core.Locale.lookupTranslation(s, 'he')).join('־')
    },
...
function findParshaHaShavua(saturday, il) {
  ...
  const sedra = core.HebrewCalendar.getSedra(hyear, il);
  const parsha = sedra.lookup(saturday);
  if (!parsha.chag) {
    return parsha;
  }
  // Search for next regular parsha, which could even spill into next year
  const endOfYear = new core.HDate(1, core.months.TISHREI, hyear + 1).abs() - 1;
  const endAbs = endOfYear + 30;
  for (let sat2 = saturday.abs() + 7; sat2 <= endAbs; sat2 += 7) {
    const sedra2 = sat2 > endOfYear ? core.HebrewCalendar.getSedra(hyear + 1, il) : sedra;
...
function getLeyningOnDate(hdate, il, wantarray = false) {
  ...
  if (dow === 6 && !parsha.chag) {
    const parshaEvent = new core.ParshaEvent(hdate, parsha.parsha, il);
    const reading = getLeyningForParshaHaShavua(parshaEvent, il);
    return wantarray ? [reading] : reading;
  }
  const events = core.HebrewCalendar.getHolidaysOnDate(hdate, il) || [];
  const arr = [];
...
function writeFullKriyahCsv(stream, hyear, il) {
  const events0 = core.HebrewCalendar.calendar({
    year: hyear,
    isHebrewYear: true,
    sedrot: true,
    il: il
...
function writeFullKriyahEvent(stream, ev, il) {
  ...
  const mask = ev.getFlags();
  const isParsha = mask === core.flags.PARSHA_HASHAVUA;
...
function writeHolidayMincha(stream, ev, il) {
  ...
  if (readingMincha) {
    const minchaEv = new core.Event(ev.getDate(), minchaDesc1, core.flags.USER_EVENT);
